...
class CustomerReportService {
  ...
  static Future<void> sharePdf({required String namaPelanggan, required List<TransaksiKredit> transaksi, required Map<int, List<Pembayaran>> pembayaran}) async {
    ...
    final file = File(p.join(dir.path, 'laporan_${safe}_${DateTime.now().millisecondsSinceEpoch}.pdf'));
    await file.writeAsBytes(await doc.save());
    await SharePlus.instance.share(ShareParams(files: [XFile(file.path)], text: 'Laporan transaksi pelanggan $namaPelanggan'));
  }
  ...
  static pw.Widget _table(List<TransaksiKredit> ts, Map<int, List<Pembayaran>> p) {
    final rows = <pw.TableRow>[
      pw.TableRow(repeat: true, decoration: const pw.BoxDecoration(color: PdfColors.grey200), children: ['No', 'Tanggal', 'Resi', 'Penerima', 'Kota', 'Jumlah'].map((v) => pw.Padding(
        padding: const pw.EdgeInsets.all(4),
        child: pw.Text(v, textAlign: v == 'Jumlah' ? pw.TextAlign.right : pw.TextAlign.left, style: pw.TextStyle(fontSize: 7, fontWeight: pw.FontWeight.bold)),
      )).toList()),
    ];
    ...
    return pw.Table(border: pw.TableBorder.all(color: PdfColors.grey500, width: .5), columnWidths: const {
      0: pw.FixedColumnWidth(22), 1: pw.FixedColumnWidth(55), 2: pw.FlexColumnWidth(1.15), 3: pw.FlexColumnWidth(1.25), 4: pw.FlexColumnWidth(1.35), 5: pw.FixedColumnWidth(72),
    }, children: rows);
  }

  static pw.Widget _c(String v, {bool right = false, bool bold = false}) => pw.Padding(
    padding: const pw.EdgeInsets.symmetric(horizontal: 4, vertical: 4),
    child: pw.Text(v, textAlign: right ? pw.TextAlign.right : pw.TextAlign.left, style: pw.TextStyle(fontSize: 7.2, fontWeight: bold ? pw.FontWeight.bold : pw.FontWeight.normal), maxLines: 3),
  );
  ...
  static Future<void> shareExcel({required String namaPelanggan, required List<TransaksiKredit> transaksi, required Map<int, List<Pembayaran>> pembayaran}) async {
    final settings = await ReportHeaderSettings.load();
    final sorted = List<TransaksiKredit>.from(transaksi)..sort((a, b) => a.tanggal.compareTo(b.tanggal));
    final total = sorted.fold<int>(0, (s, t) => s + t.jumlah);
    final paid = sorted.fold<int>(0, (s, t) => s + _paidFor(t, pembayaran));
    ...
    s.appendRow([]);
    s.appendRow([TextCellValue('No'), TextCellValue('Tanggal'), TextCellValue('Resi'), TextCellValue('Penerima'), TextCellValue('Kota'), TextCellValue('Jumlah')]);
    ...
    final dir = await getTemporaryDirectory();
    final safe = namaPelanggan.replaceAll(RegExp(r'[^A-Za-z0-9_-]+'), '_');
    final f = File(p.join(dir.path, 'laporan_${safe}_${DateTime.now().millisecondsSinceEpoch}.xlsx'));
    await f.writeAsBytes(bytes);
    await SharePlus.instance.share(ShareParams(files: [XFile(f.path)], text: 'Laporan transaksi pelanggan $namaPelanggan'));
  }
}
