...
enum CustomerReportType { summary, complete }

class CustomerReportService {
  static int _paidFor(TransaksiKredit t, Map<int, List<Pembayaran>> pembayaran) =>
  ...
  static Future<void> sharePdf({
    ...
    required CustomerReportType type,
  }) async {
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        build: (_) => [
          _header(settings.companyName, settings.reportTitle, logo),
          ...
          pw.Text(
            type == CustomerReportType.summary ? 'LAPORAN TRANSAKSI' : 'LAPORAN TRANSAKSI & PIUTANG',
            style: pw.TextStyle(fontSize: 11, fontWeight: pw.FontWeight.bold),
          ),
          pw.SizedBox(height: 6),
          _table(sorted, pembayaran, type),
          pw.SizedBox(height: 12),
          _summary(total, paid, remaining, type),
          if (type == CustomerReportType.complete) ...[
            pw.SizedBox(height: 12),
            pw.Text('Terbilang sisa: ${_terbilang(remaining)} rupiah', style: pw.TextStyle(fontSize: 9, fontWeight: pw.FontWeight.bold)),
          ],
          pw.SizedBox(height: 14),
    ...
    await SharePlus.instance.share(
      ShareParams(files: [XFile(file.path)], text: 'Laporan ${suffix} pelanggan $namaPelanggan'),
    );
  }
  ...
  static pw.Widget _header(String companyName, String reportTitle, pw.MemoryImage? logo) =>
      pw.Container(
        padding: const pw.EdgeInsets.only(bottom: 12),
        decoration: const pw.BoxDecoration(border: pw.Border(bottom: pw.BorderSide(width: 1.2))),
        child: pw.Row(children: [
          if (logo != null)
            pw.Container(width: 58, height: 58, padding: const pw.EdgeInsets.only(right: 10), child: pw.Image(logo, fit: pw.BoxFit.contain)),
          pw.Expanded(
  ...
  static pw.Widget _table(
    ...
    final rows = <pw.TableRow>[
      pw.TableRow(
        ...
        children: headers.map((v) => pw.Padding(
          padding: const pw.EdgeInsets.all(4),
          child: pw.Text(v, textAlign: _rightHeader(v) ? pw.TextAlign.right : pw.TextAlign.left, style: pw.TextStyle(fontSize: 7, fontWeight: pw.FontWeight.bold)),
        )).toList(),
      ),
    ];
    ...
    for (var i = 0; i < ts.length; i++) {
      ...
      rows.add(
        pw.TableRow(
          children: type == CustomerReportType.summary
              ? [
                  _c('${i + 1}'),
                  ...
                  _c(t.namaPenerima),
                  _c(t.kotaTujuan),
                  _c('${t.quantity}', right: true),
                  _c(_formatBerat(t.berat), right: true),
                  _c(Formatter.rupiah(t.jumlah), right: true),
                ]
              : [
                  _c('${i + 1}'),
                  _c(Formatter.tanggalPendek(t.tanggal)),
                  ...
                  _c(t.kotaTujuan),
                  _c(Formatter.rupiah(t.jumlah), right: true),
                  _c(Formatter.rupiah(paid), right: true),
                  _c(Formatter.rupiah(sisa), right: true),
                ],
        ),
      );
    }
    ...
    if (type == CustomerReportType.summary) {
      rows.add(pw.TableRow(
        ...
        children: [
          ...
          _c('${ts.fold<int>(0, (s, t) => s + t.quantity)}', right: true, bold: true),
          _c(_formatBerat(ts.fold<double>(0, (s, t) => s + t.berat)), right: true, bold: true),
    ...
    return pw.Table(
      ...
      columnWidths: type == CustomerReportType.summary
          ? const {
              0: pw.FixedColumnWidth(20), 1: pw.FixedColumnWidth(50), 2: pw.FlexColumnWidth(1.0), 3: pw.FlexColumnWidth(1.15),
              4: pw.FlexColumnWidth(1.15), 5: pw.FixedColumnWidth(30), 6: pw.FixedColumnWidth(42), 7: pw.FixedColumnWidth(68),
            }
          : const {
              0: pw.FixedColumnWidth(20), 1: pw.FixedColumnWidth(50), 2: pw.FlexColumnWidth(1.0), 3: pw.FlexColumnWidth(1.15),
              4: pw.FlexColumnWidth(1.15), 5: pw.FixedColumnWidth(68), 6: pw.FixedColumnWidth(68), 7: pw.FixedColumnWidth(68),
            },
      children: rows,
    );
  }

  static bool _rightHeader(String value) => const {'Qty', 'Berat', 'Total Transaksi', 'Dibayar', 'Sisa'}.contains(value);
  ...
  static pw.Widget _summary(int total, int paid, int remaining, CustomerReportType type) {
    if (type == CustomerReportType.summary) {
    ...
    return pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
      pw.Text('RINGKASAN PIUTANG', style: pw.TextStyle(fontSize: 9, fontWeight: pw.FontWeight.bold)),
      pw.SizedBox(height: 4),
      pw.Text('Total transaksi: ${Formatter.rupiah(total)}'),
  ...
  static pw.Widget _c(String v, {bool right = false, bool bold = false}) => pw.Padding(
        ...
        child: pw.Text(
          ...
          style: pw.TextStyle(fontSize: 7.0, fontWeight: bold ? pw.FontWeight.bold : pw.FontWeight.normal),
          maxLines: 3,
        ),
      );
  ...
  static Future<void> shareExcel({
    ...
    s.appendRow([TextCellValue('Jenis Laporan'), TextCellValue(type == CustomerReportType.summary ? 'Ringkas - Total Transaksi' : 'Lengkap - Transaksi & Piutang')]);
    ...
    if (type == CustomerReportType.summary) {
      s.appendRow([TextCellValue('No'), TextCellValue('Tanggal'), TextCellValue('Resi'), TextCellValue('Penerima'), TextCellValue('Kota'), TextCellValue('Qty'), TextCellValue('Berat (kg)'), TextCellValue('Total Transaksi')]);
      ...
      s.appendRow([TextCellValue(''), TextCellValue(''), TextCellValue(''), TextCellValue(''), TextCellValue('TOTAL'), IntCellValue(sorted.fold<int>(0, (s, t) => s + t.quantity)), DoubleCellValue(sorted.fold<double>(0, (s, t) => s + t.berat)), IntCellValue(total)]);
      ...
      for (var i = 0; i < sorted.length; i++) {
        final t = sorted[i];
        final paidFor = _paidFor(t, pembayaran);
        final sisa = (t.jumlah - paidFor).clamp(0, t.jumlah);
        s.appendRow([IntCellValue(i + 1), TextCellValue(Formatter.tanggalPendek(t.tanggal)), TextCellValue(t.nomorResi), TextCellValue(t.namaPenerima), TextCellValue(t.kotaTujuan), IntCellValue(t.jumlah), IntCellValue(paidFor), IntCellValue(sisa)]);
      }
    ...
    await SharePlus.instance.share(
      ShareParams(files: [XFile(f.path)], text: 'Laporan ${suffix} pelanggan $namaPelanggan'),
    );
  }
}
