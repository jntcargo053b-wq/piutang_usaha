...
class ReportHeaderSettings {
  ...
  static Future<ReportHeaderSettings> load() async {
    ...
    return ReportHeaderSettings(
      companyName: prefs.getString(_companyKey) ?? defaultCompany,
      reportTitle: prefs.getString(_titleKey) ?? defaultTitle,
      logoPath: prefs.getString(_logoKey),
    );
  }

  Future<void> save({String? companyName, String? reportTitle, String? logoPath}) async {
    final prefs = await SharedPreferences.getInstance();
