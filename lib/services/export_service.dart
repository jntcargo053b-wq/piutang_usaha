...
class ExportService {
    ...
    doc.addPage(pw.MultiPage(
      ...
      build: (_) => [
        ...
        pw.Container(
          ...
          child: pw.Row(children: [
            pw.Expanded(child: pw.Text('TOTAL', style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold))),
  ...
  static pw.Widget _totalCell(int value, {double width = 78}) => pw.SizedBox(
        width: width,
        child: pw.Text(Formatter.rupiah(value), textAlign: pw.TextAlign.right, style: pw.TextStyle(fontSize: 8, fontWeight: pw.FontWeight.bold)),
      );
  ...
  static Future<void> exportRekapKeExcel(List<Map<String, dynamic>> rows) async {
    ...
    sheet.appendRow([
      ...
      TextCellValue(Formatter.tanggalPendek(rows.isEmpty ? DateTime.now() : _parseDate(rows.first['tanggal']))),
      TextCellValue('-'),
      TextCellValue(Formatter.tanggalPendek(rows.isEmpty ? DateTime.now() : _parseDate(rows.last['tanggal']))),
    ]);
    sheet.appendRow([]);
    ...
    for (final row in rows) {
      ...
      totalDibayarPeriode += dibayarPeriode;
      totalSisa += sisa;
      sheet.appendRow([TextCellValue('${row['tanggal'] ?? ''}'), TextCellValue('${row['nama_pelanggan'] ?? ''}'), TextCellValue('${row['nomor_resi'] ?? ''}'), TextCellValue('${row['nama_penerima'] ?? ''}'), TextCellValue('${row['kota_tujuan'] ?? ''}'), IntCellValue(kredit), IntCellValue(dibayar), IntCellValue(dibayarPeriode), IntCellValue(sisa)]);
      for (var column = 5; column <= 8; column++) {
        sheet.cell(CellIndex.indexByColumnRow(columnIndex: column, rowIndex: excelRow)).cellStyle = numberStyle;
      }
      excelRow++;
    }
    sheet.appendRow([TextCellValue(''), TextCellValue(''), TextCellValue(''), TextCellValue(''), TextCellValue('TOTAL'), IntCellValue(totalKredit), IntCellValue(totalDibayar), IntCellValue(totalDibayarPeriode), IntCellValue(totalSisa)]);
    for (var column = 5; column <= 8; column++) {
      sheet.cell(CellIndex.indexByColumnRow(columnIndex: column, rowIndex: excelRow)).cellStyle = numberStyle;
    }
    ...
    await SharePlus.instance.share(ShareParams(files: [XFile(file.path)], text: 'Laporan Piutang Usaha Excel'));
  }
