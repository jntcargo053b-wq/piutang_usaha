...
class _PelangganScreenState extends State<PelangganScreen> {
        ...
        builder: (ctx, setSheetState) => Padding(
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                Text(old == null ? 'Tambah Pelanggan' : 'Edit Pelanggan', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
  ...
  Future<void> _delete(Pelanggan pelanggan) async {
    ...
    try {
      ...
      await provider.hapusPelanggan(pelanggan.id!);
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('$e')));
    }
  }

  Future<void> _openDetail(BuildContext itemContext, Pelanggan pelanggan) async {
    final navigator = Navigator.of(itemContext);
    await navigator.push(MaterialPageRoute(builder: (_) => DetailPelangganScreen(pelanggan: pelanggan)));
    if (!mounted) return;
    final provider = context.read<PiutangProvider>();
    ...
    return Scaffold(
      ...
      body: Consumer<PiutangProvider>(
        builder: (context, provider, _) {
          ...
          if (provider.daftarPelanggan.isEmpty) return const Center(child: Text('Belum ada pelanggan.'));
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return ListTile(
                ...
                trailing: Text(Formatter.rupiah(sisa), style: TextStyle(fontWeight: FontWeight.bold, color: sisa > 0 ? Colors.red : Colors.green)),
                onTap: () => _openDetail(context, pelanggan),
                onLongPress: () => showModalBottomSheet<void>(
                  context: context,
                  builder: (sheetContext) => SafeArea(
                    child: Wrap(children: [
                      ListTile(leading: const Icon(Icons.edit), title: const Text('Edit'), onTap: () { Navigator.pop(sheetContext); _form(old: pelanggan); }),
                      ListTile(leading: const Icon(Icons.delete), title: const Text('Hapus'), onTap: () { Navigator.pop(sheetContext); _delete(pelanggan); }),
                    ]),
                  ),
                ),
              );
