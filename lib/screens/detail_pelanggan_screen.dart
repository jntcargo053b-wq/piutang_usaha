...
class _DetailPelangganScreenState extends State<DetailPelangganScreen> {
      ...
      await showModalBottomSheet<void>(
        ...
        builder: (sheetContext) => SafeArea(
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ListTile(leading: const Icon(Icons.picture_as_pdf), title: const Text('Laporan PDF'), subtitle: const Text('Daftar transaksi pelanggan'), onTap: () async { Navigator.pop(sheetContext); await CustomerReportService.sharePdf(namaPelanggan: widget.pelanggan.nama, transaksi: rows, pembayaran: pembayaran); }),
            ListTile(leading: const Icon(Icons.table_chart), title: const Text('Laporan Excel'), subtitle: const Text('Daftar transaksi pelanggan'), onTap: () async { Navigator.pop(sheetContext); await CustomerReportService.shareExcel(namaPelanggan: widget.pelanggan.nama, transaksi: rows, pembayaran: pembayaran); }),
          ]),
        ),
      );
    } catch (e) {
            ...
            child: Form(
              ...
              child: Column(children: [
                ...
                TextFormField(
                  controller: city,
                  readOnly: true,
                  decoration: const InputDecoration(labelText: 'Kota Tujuan *', hintText: 'Pilih kabupaten/kota', suffixIcon: Icon(Icons.arrow_drop_down)),
                  onTap: () async {
                    final selected = await _pilihKota(city.text);
                    if (selected != null) setSheetState(() => city.text = selected);
                  },
                ...
                SizedBox(width: double.infinity, child: ElevatedButton(onPressed: saving ? null : () async {
                  ...
                  try {
                    await ctx.read<PiutangProvider>().tambahTransaksi(TransaksiKredit(pelangganId: widget.pelanggan.id!, tanggal: date, nomorResi: res.text.trim(), namaPenerima: recv.text.trim(), kotaTujuan: city.text.trim(), jumlah: int.parse(amount.text.replaceAll('.', '')), catatan: note.text.trim().isEmpty ? null : note.text.trim()));
                    if (ctx.mounted) Navigator.pop(ctx);
                    if (mounted) await _load();
                  } catch (e) {
                    if (ctx.mounted) { setSheetState(() => saving = false); ScaffoldMessenger.of(ctx).showSnackBar(SnackBar(content: Text('$e'))); }
                  }
  ...
  Future<void> _bayar(TransaksiKredit transaksi) async {
    ...
    await showDialog<void>(context: context, builder: (ctx) => StatefulBuilder(builder: (ctx, setDialogState) => AlertDialog(
      ...
      actions: [TextButton(onPressed: saving ? null : () => Navigator.pop(ctx), child: const Text('Batal')), ElevatedButton(onPressed: saving ? null : () async {
        final value = int.tryParse(amount.text.replaceAll('.', ''));
        if (value == null || value <= 0 || value > transaksi.sisa) { ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(content: Text('Jumlah pembayaran tidak valid'))); return; }
        setDialogState(() => saving = true);
        try {
          await ctx.read<PiutangProvider>().tambahPembayaran(Pembayaran(transaksiId: transaksi.id!, tanggal: DateTime.now(), jumlah: value, keterangan: note.text.trim().isEmpty ? null : note.text.trim()));
          if (ctx.mounted) Navigator.pop(ctx);
          if (mounted) await _load();
        } catch (e) { if (ctx.mounted) { setDialogState(() => saving = false); ScaffoldMessenger.of(ctx).showSnackBar(SnackBar(content: Text('$e'))); } }
...
class _CityPickerState extends State<_CityPicker> {
  ...
  Widget build(BuildContext context) {
    ...
    final results = q.isEmpty ? indonesiaCities : indonesiaCities.where((c) => c.toLowerCase().contains(q)).toList();
    return SafeArea(child: Padding(padding: EdgeInsets.only(bottom: MediaQuery.of(context).viewInsets.bottom), child: Column(children: [
