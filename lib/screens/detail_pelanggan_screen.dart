...
class _DetailPelangganScreenState extends State<DetailPelangganScreen> {
              ...
              child: Form(
                ...
                child: Column(
                  children: [
                    ...
                    ListTile(
                      title: Text(Formatter.tanggalPanjang(date)),
                                ...
                                try {
                                  await ctx.read<PiutangProvider>().tambahTransaksi(
                                        TransaksiKredit(
                                          ...
                                          namaPenerima: recv.text.trim(),
                                          kotaTujuan: city.text.trim(),
                                          jumlah: int.parse(amount.text.replaceAll('.', '')),
                                          catatan: note.text.trim().isEmpty ? null : note.text.trim(),
                                        ),
                                      );
              ...
              ElevatedButton(
                onPressed: saving
                    ...
                    : () async {
                        ...
                        try {
                          await ctx.read<PiutangProvider>().tambahPembayaran(
                                Pembayaran(
                                  transaksiId: transaksi.id!,
                                  tanggal: DateTime.now(),
                                  jumlah: value,
                                  keterangan: note.text.trim().isEmpty ? null : note.text.trim(),
                                ),
                              );
                          if (ctx.mounted) Navigator.pop(ctx);
                          if (mounted) await _load();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(widget.pelanggan.nama)),
