                        ...
                        try {
                          await ctx.read<PiutangProvider>().tambahPembayaran(
                            Pembayaran(
                              ...
                              jumlah: value,
                              keterangan: keterangan.text.trim().isEmpty ? null : keterangan.text.trim(),
                            ),
                          );
                          if (ctx.mounted) Navigator.pop(ctx);
                          if (mounted) await _load();
...
class _CityPickerState extends State<_CityPicker> {
  ...
  Widget build(BuildContext context) {
    ...
    final results = query.isEmpty
        ? indonesiaCities
        : indonesiaCities.where((city) => city.toLowerCase().contains(query)).toList();
          ...
          children: [
            Padding(
              ...
              child: Row(
                children: [
                  const Expanded(
                    child: Text('Pilih Kota/Kabupaten', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700)),
                  ),
