    ...
    return Scaffold(
      ...
      body: loading ? const Center(child: CircularProgressIndicator()) : rows.isEmpty
          ...
          : RefreshIndicator(onRefresh: _load, child: ListView.separated(padding: const EdgeInsets.fromLTRB(12, 12, 12, 96), physics: const AlwaysScrollableScrollPhysics(), itemCount: rows.length, separatorBuilder: (_, __) => const SizedBox(height: 8), itemBuilder: (context, index) {
              ...
              return Card(clipBehavior: Clip.antiAlias, child: ListTile(contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8), title: Text(transaksi.nomorResi, maxLines: 1, overflow: TextOverflow.ellipsis), subtitle: Padding(padding: const EdgeInsets.only(top: 6), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('${transaksi.namaPenerima} • ${transaksi.kotaTujuan}', maxLines: 2, overflow: TextOverflow.ellipsis), const SizedBox(height: 4), Text('Qty ${transaksi.quantity} • ${_formatBerat(transaksi.berat)} kg'), const SizedBox(height: 4), Text(Formatter.tanggalPanjang(transaksi.tanggal))])), trailing: PopupMenuButton<String>(onSelected: (value) { if (value == 'bayar') _bayar(transaksi); if (value == 'edit') _transaksi(existing: transaksi); if (value == 'hapus') _hapusTransaksi(transaksi); }, itemBuilder: (_) => [if (transaksi.sisa > 0) const PopupMenuItem(value: 'bayar', child: Text('Bayar')), const PopupMenuItem(value: 'edit', child: Text('Edit')), const PopupMenuItem(value: 'hapus', child: Text('Hapus'))]), leading: CircleAvatar(backgroundColor: scheme.primaryContainer, foregroundColor: scheme.onPrimaryContainer, child: Icon(transaksi.sisa <= 0 ? Icons.check : Icons.receipt_long_outlined))));
            })),
    );
  }
...
class _CityPickerState extends State<_CityPicker> {
  ...
  String query = '';
  @override
  void initState() { super.initState(); searchController = TextEditingController(text: widget.initialValue); searchController.addListener(() { if (mounted) setState(() => query = searchController.text); }); }
  @override
